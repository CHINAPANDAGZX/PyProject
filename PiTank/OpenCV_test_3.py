...
def main():
    ...
    while cv2.waitKey(1) != 27 and capWebcam.isOpened():                # until the Esc key is pressed or webcam connection is lost
        ...
        mask = cv2.inRange(imgHSV,low_white,high_white)
        res = cv2.bitwise_and(imgOriginal,imgOriginal,mask=mask)
        img = cv2.medianBlur(imgOriginal,5)
        cimg = cv2.cvtColor(img,cv2.COLOR_GRAY2BGR)
        #print res.shape
        #imgThresh = cv2.add(imgThreshLow, imgThreshHigh)

        #imgThresh = cv2.GaussianBlur(res, (3, 3), 2)

        #imgThresh = cv2.dilate(imgThresh, np.ones((5,5),np.uint8))
        #imgThresh = cv2.erode(imgThresh, np.ones((5,5),np.uint8))

        #intRows, intColumns = img.shape

        circles = cv2.HoughCircles(cimg, cv2.HOUGH_GRADIENT, 10, 20,50,30,10,50)      # fill variable circles with all circles in the processed image
